...
for j in range(0,11):
    ...
    for i in range(1000):
        ...
        new_infected_succeptible = np.random.choice(range(2), Susceptible, p=[1-up_beta, up_beta])
        new_infected_vaccinated = np.random.choice(range(2), Vaccinated, p=[1-delta, delta])
        new_infected = np.concatenate((new_infected_succeptible, new_infected_vaccinated))
